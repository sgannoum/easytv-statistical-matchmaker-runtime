...
var MAX_Distance = process.env.MAX_DISTANCE || 0.3;
...
class StatisticalMatchMakerImpl {
		 ...
		 */
		personalize_profile(user_id, user_profile, maxDist = MAX_Distance) {
			...
			//Addative inference
			new_user_profile = this.addativeInference(new_user_profile, relevantClusters, maxDist);
		 ...
		 */
		personalize_context(user_id, user_profile, user_context, maxDist = MAX_Distance) {
		 ...
		 */
		personalize_content(user_id, user_profile, user_context, user_content, maxDist = MAX_Distance) {
		 ...
		 */
		addativeInference(user_profile, relevant_clusters, max_distance = 0.3) {
			...
			for (var i in relevant_clusters) 
			{
				...
				if(relevant_entry.distance >= max_distance)
					break
